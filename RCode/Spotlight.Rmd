...
country_all <- plyr::rbind.fill(country_inform,country_wbank) %>%
  plyr::join(., Rank_class_df, by="Rank_class") %>%
  mutate(Group = case_when(grepl("VU|MORT|VUL|EMP|UEM ", Index) == TRUE ~ "c) Vulnerability",
         grepl("HA|FRST|PRCP|ATM| ", Index) == TRUE ~ "b) Hazards & Exposure", grepl("CC", Index) == TRUE ~ "d) Lack fo Coping Capacity", 
          grepl("INFORM", Index) == TRUE ~ "a) Risk Index")) %>%
  mutate(Description = " ") %>%
  arrange(Group)
...
```{r, echo=FALSE,message=FALSE, out.width="20%", fig.align = "right"}
library(geodata)
