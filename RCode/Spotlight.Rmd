...
country_inform <- lapply(Country_with_ranks, function(x) x[x$iso == country_iso,]) %>% #remove the iso column
  ...
  mutate(Source = "DRMKC INFORM, Mid 2022") #adapt as variable!
...
country_all <- plyr::rbind.fill(country_inform,country_wbank) %>%
  mutate(Group = case_when(grepl("VU|MORT|VUL|EMP|UEM ", Index) == TRUE ~ "VU",
         grepl("HA|FRST|PRCP|ATM| ", Index) == TRUE ~ "HA", grepl("CC", Index) == TRUE ~ "CC", 
          grepl("INFORM", Index) == TRUE ~ "Risk")) #unfinished
