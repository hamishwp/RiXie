...
```{r  country and flag , message=FALSE,fig.align="right", out.width="20%"}
library(geodata)
library(ggflags)
library(ggplot2)
library(ggthemes)
...
country_all[c("Indicator Name","Description","Value","Year","Rank","Comparative Value","Source")] %>%
  ...
  column_spec(c(2,7), width_min = c("8cm","5cm"), width_max = c("8cm","5cm")) %>%
...
#Subest table
var_select <-c("Category", "Description", "Mean_Value", "Rank", "Rank_out_of","Class_levels", "Year_of_Publication", "Organisation", "URL")
#Grouping names
...
#----------------------------------TABLE-----------------------------------------------------------
#tile color
adm_country$`Comparative Value` <- color_tile2(c("palegreen3",'lightgoldenrod1', "tomato2"))(adm_country$`Comparative Value`)
  
  
#table shapefile
adm_country[c(2,3,4,6,7,8)] %>%
  kbl(escape=F) %>%
  kable_paper("striped", full_width = F) %>%
