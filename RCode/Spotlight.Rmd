...
#population codes for number formatiing:
...
adm_country <- lapply(adm_ranks, function(x) t(x[x$iso == country_iso,][-1])[,1]) %>%
  ...
  plyr::join(., Rank_class_df, by="Rank_class") %>%
  mutate(Mean_Value = ifelse(Codes %in% pop, sub("\\.\\d+$", "",Mean_Value), round(Mean_Value,2))) %>%
...
all<-plyr::rbind.fill(Wbank_inform_combi,adm_country) %>%
  ...
  mutate(Unit = case_when(grepl("scale|count|RP|=|constant|aggregate|-2011", Unit) ==FALSE ~ gsub("[()]", "", Unit),
            TRUE ~ Unit)) %>%
  mutate(Description = as.character(paste0("The ", tolower(Index_name)," for ", country_full," is estimated to be ",
                                           format(Value,big.mark=",")," ",Unit,", which is considered ", tolower(`Comparative Value`)," value as compared to ",
...
#write
write.csv(all,file = paste0("/home/coleen/Documents/GRAF_files/Results/Spotlight/",country_iso,"_RiX-Spotlight.csv"),row.names = FALSE)
```
