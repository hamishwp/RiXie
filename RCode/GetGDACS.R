  ...
  for (alert in alertlist){
    ...
    for (ev in haz){
      ...
      for (year in GDACS_yrs){
        ...
        if(!typeof(tGDACS)=="list" | class(tGDACS)=="try-error"){
...
FilterGDACS<-function(haz=NULL,syear=2016L,fyear=2020L,iso3c=NULL,list_GDACS=NULL,red=F){
  ...
  for (i in 1:length(list_GDACS)){
    ...
    if(!is.null(iso3c)) {
      if(tmp$properties$iso3!=iso3c) next
    }
    ...
    for (j in 1:length(tmp$properties$episodealertlevel)){
      ...
      dfGDACS_save<-dfGDACS
      dfGDACS<-tryCatch(rbind(dfGDACS,data.frame(alert=rep(trimws(tolower(tmp$properties$episodealertlevel[j]), "b"),len),
                                        ...
                                        # ifelse(length(txt)!=len,rep(txt,len),txt),
                                        geom_type=rep(tmp$geometry$type,len),
                                        long=rep(tmp$geometry$coordinates[1],len),
                                        lat=rep(tmp$geometry$coordinates[2],len),
                                        geom_link=rep(tmp$properties$url$geometry,len))),error=function(e) NA)
      if(class(dfGDACS)!="data.frame" | is.na(dfGDACS)) {
        print(paste0("Error at i= ",i,", j = ",j))
        return(list(dfGDACS_save,tmp))
        stop()
      }
    }
...
GetShakeGDACS<-function(dfGDACS,hazard="EQ",directory,plotty=FALSE){
  ...
  while (length(dfGDACS$link)>0){
    ...
    evs%<>%filter(eventid %in% ids & hazard_severity>Mc & eventid!=mev)
